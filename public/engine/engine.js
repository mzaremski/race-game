...
var socket = io("http://192.168.1.36:8000");
	...
	gameLoop() {
		...
		if (Game.simulateFRAME) {
			for (let i in players) {
				const vehicle = players[i].vehicle
				const { modX, modY, modAngle } = players[i]
				if (vehicle) {
					vehicle.position[0] += modX / 2;
					vehicle.position[1] += modY / 2;
					vehicle.angle += modAngle / 2;
				}
			}
		}
		...
		// VAR.scale =
		// VAR.defaultScale *
		// (1.04 - Math.abs(players[socket.nick].vehicle.velocity["1"] * 0.00015));
		// Map.scale(VAR.scale);
